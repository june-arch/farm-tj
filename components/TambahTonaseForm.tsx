...
export default function TambahTonaseForm() {
  ...
  useEffect(() => {
    const calculatePemasukan = debounce(() => {
      if (!netto) {
        return; // Handle error separately
      }
      setPemasukan(hargaJual * netto);
    }, 500);

    calculatePemasukan();

    return () => {
      calculatePemasukan.cancel();
    };
  }, [hargaJual, netto]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("api/struks/", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        toast.success("Tonase created successfully");
